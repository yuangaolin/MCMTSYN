...
def test(model, drugEncoder, cellLineEncoder, test_dataloader, test_num, mse, cce, test_fold):
    ...
    with torch.no_grad():
        ...
        output_file = 'autodl-tmp/predictions.txt'
        # 写入文件
...
cellLineAE.eval()
print(
    "--------- MCMTSYN with drugAE(" + str(DrugAE_OutputDim) + ") cellLineAE(" + str(CellAE_OutputDim) + ") ---------")
with open(MCMTSYN_Result, 'a') as file:
    file.write("--------- MCMTSYN with drugAE(" + str(DrugAE_OutputDim) + ") cellLineAE(" + str(
        CellAE_OutputDim) + ") ---------\n")
result_per_fold = []
